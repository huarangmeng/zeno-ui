...
- `zeno-scene` 负责 `Scene`、`DrawCommand`、`Renderer` trait 与 backend probe 契约。
- `zeno-ui` 负责声明式节点树、modifier、retained tree、布局与从节点树到 `SceneSubmit` 的转换。
...
- `zeno-runtime` 已具备平台无关的 `App/AppFrame/AppView/AppHost + UiRuntime` 主干，并直接持有 `run_app` 入口；`zeno-platform` 只提供桌面/移动宿主与 session builder，外部不再直接感知 `SceneSubmit + FrameRequest` 等内部提交/调度协议。
