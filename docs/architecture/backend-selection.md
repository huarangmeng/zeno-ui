...
## 状态
- 状态：进行中
- 阶段判断：后端优先级、fallback 策略与统一 `ResolvedSession` 描述符已经成立；当前缺口主要收敛为“非 macOS 的 Impeller presenter 尚未实现”与“移动端 presenter 的 GPU 生命周期仍未完全原生化”，而协议层已经收敛为 retained-only 提交模型。
...
- `Renderer` trait 与 presenter/session 主链都已切到 retained-only；macOS Impeller 不再维护旧 `render(&Scene)` 主路径，移动端 presenter 也不再通过 `snapshot_scene()` 中转。
