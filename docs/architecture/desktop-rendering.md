...
- 图片资源也已对齐到 display-list-first 主链：`ImageNode` 通过 `ImageSource + ImageResourceTable` 进入 builder，再分别解析成带稳定 `cache_key` 的 `DisplayImage` / `DrawCommand::Image` 供桌面 Skia 与 macOS Impeller 消费。
...
- 图片缓存已进入 Skia 主链：`DisplayImage.cache_key` 会驱动 Skia 复用 `sk::Image`，避免同图在多 tile / 多帧场景下重复 `new_copy + raster_from_data`。
...
- 图片纹理缓存已经进入主链：`DisplayImage.cache_key` 会驱动 Impeller 复用 Metal texture，避免同图在多 tile / 多帧场景下重复上传。
- 离屏 context 已接入 `OffscreenContextCache`，可在稳定 stacking context 场景中复用已有 offscreen texture。
- `DisplayList` 热路径已补 render-time lookup table：一次性构建 `spatial transform / clip chain / stacking context / context bounds` 索引，减少 tile raster/composite 期间的线性查找。
...
- 当前剩余重点已转向更完整的 compositor 基础设施：跨 backend 更细粒度的 GPU tile 资源复用/淘汰策略、backend 直接消费更完整 layer/effect graph 的 GPU 执行器、独立 compositor 层、更复杂 effect graph、图片多来源/解码缓存，以及非 macOS Impeller presenter。
...
- 把当前主要在 macOS Impeller 主链存在的离屏缓存与 render-time lookup 经验继续抽象为跨 backend 可复用能力，同时继续增强 Skia 侧与之对齐的统计与缓存策略。
