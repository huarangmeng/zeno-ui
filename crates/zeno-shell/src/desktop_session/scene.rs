...
pub(super) fn partial_scene_for_dirty_bounds(scene: &Scene, dirty_bounds: Rect) -> Scene {
    ...
    let layers_by_id: HashMap<u64, &SceneLayer> =
        layers.iter().map(|layer| (layer.layer_id, layer)).collect();
    ...
    blocks.extend(
        scene
            .blocks
            .iter()
            .filter(|block| partial_scene_should_include_block(block, &layers_by_id, dirty_bounds))
            .cloned()
            .enumerate()
            .map(|(index, mut block)| {
...
fn partial_layers_for_dirty_bounds(scene: &Scene, dirty_bounds: Rect) -> Vec<SceneLayer> {
    ...
    for layer_id in scene
        .layers
        .iter()
        .filter(|layer| layer.bounds.intersects(&dirty_bounds))
...
fn partial_scene_should_include_block(
    ...
    let Some(layer) = layers_by_id.get(&block.layer_id).copied() else {
        return false;
    };
    block.bounds.intersects(&dirty_bounds)
        || (layer.bounds.intersects(&dirty_bounds) && layer_requires_full_block_replay(layer))
}

#[cfg(all(target_os = "macos", feature = "desktop_winit"))]
fn layer_requires_full_block_replay(layer: &SceneLayer) -> bool {
    layer.offscreen || layer.clip.is_some() || layer.opacity < 1.0 || !layer.effects.is_empty()
}
...
mod tests {
    ...
    use zeno_graphics::{
        Brush, DrawCommand, Scene, SceneBlendMode, SceneBlock, SceneLayer, SceneSubmit,
    ...
    fn patch_stats_report_full_and_patch_counts() {
        ...
        let patch = super::patch_stats(&SceneSubmit::Patch {
            patch: zeno_graphics::ScenePatch {
                size: scene.size,
