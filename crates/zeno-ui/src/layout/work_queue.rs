...
use crate::frontend::{
    FrontendObjectKind, FrontendObjectTable, compile_object_table,
};
use crate::layout::{
...
impl LayoutWorkQueue {
    ...
    pub fn push(&mut self, task: LayoutTask) { self.tasks.push_back(task); }
    pub fn pop(&mut self) -> Option<LayoutTask> { self.tasks.pop_back() }
}
    ...
    while let Some(task) = q.pop() {
        match task {
            LayoutTask::Measure {
                ...
                origin,
                available,
            } => measure_task(index, origin, available, objects, text_system, &mut arena, &mut q),
            LayoutTask::FinalizeContainer {
                index,
                origin,
                available,
                child_index,
            } => finalize_container(index, origin, available, child_index, objects, &mut arena),
            LayoutTask::FinalizeBox {
                index,
                origin,
                available,
                child_indices,
            } => finalize_box(index, origin, available, &child_indices, objects, &mut arena),
...
fn measure_task(
    ...
    match &object.kind {
        FrontendObjectKind::Text(text) => {
            ...
            arena.upsert(
                index,
                zeno_core::Rect::new(origin.x, origin.y, size.width, size.height),
                Some(layout),
            );
        }
        FrontendObjectKind::Spacer(spacer) => {
            ...
            arena.upsert(
                index,
                zeno_core::Rect::new(origin.x, origin.y, width, height),
                None,
            );
        }
        FrontendObjectKind::Container => {
...
fn finalize_container(
    ...
    arena.upsert(index, zeno_core::Rect::new(origin.x, origin.y, size.width, size.height), None);
}
...
fn finalize_box(
    ...
    for (&child_index, child_layout) in child_indices.iter().zip(child_layouts.iter()) {
        ...
        shift_subtree(index_shift_root(child_index), objects, aligned_origin.x - child_layout.frame.origin.x, aligned_origin.y - child_layout.frame.origin.y, arena);
    }
    arena.upsert(index, zeno_core::Rect::new(origin.x, origin.y, size.width, size.height), None);
}
...
fn index_shift_root(index: usize) -> usize {
    index
}
...
fn remaining_available_for_axis(available: Size, used_main: f32, axis: Axis) -> Size {
    match axis {
